...
public class AttributeJoinTest {
	@Test
	public void test() throws ActionException {
		...
		for (int i = 0; i < separators.length(); i++) {
			...
			ctx.getDynamicObject().getAtt().put(orig.getName(), orig);
			...
			orig.setValue(finalString);

			action.apply(ctx);
			DynamicValue result = ctx.getDynamicObject().getAtt().get(attributeName);
