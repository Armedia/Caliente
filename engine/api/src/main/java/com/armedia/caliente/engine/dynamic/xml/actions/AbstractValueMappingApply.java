...
public abstract class AbstractValueMappingApply<E extends Enum<E>> extends ConditionalAction {
	...
	protected abstract CmfArchetype getMappingType(E type);
	...
	private String mapValue(DynamicElementContext ctx, CmfArchetype mappingType, String mappingName, String sourceValue,
		CmfValueType targetType) throws ActionException {
		Mapping m = ctx.getAttributeMapper().getTargetMapping(mappingType, mappingName, sourceValue);
		return (m != null ? m.getTargetValue() : null);
	}

	private void applyMapping(DynamicElementContext ctx, CmfArchetype type, String mappingName, DynamicValue candidate)
	...
	protected void executeAction(DynamicElementContext ctx) throws ActionException {
		final CmfArchetype type = Tools.coalesce(getMappingType(getType()), ctx.getDynamicObject().getType());
