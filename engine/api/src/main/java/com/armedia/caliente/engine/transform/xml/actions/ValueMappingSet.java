...
public class ValueMappingSet extends ConditionalAction {
	...
	protected void applyTransformation(TransformationContext ctx) {
		...
		if (from == null) { throw new RuntimeTransformationException("Must provide a source value to map from"); }
		String to = Tools.toString(Expression.eval(getTo(), ctx));
		if (to == null) { throw new RuntimeTransformationException("Must provide a target value map into"); }
