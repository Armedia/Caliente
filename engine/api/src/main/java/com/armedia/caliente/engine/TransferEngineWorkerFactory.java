...
public abstract class TransferEngineWorkerFactory< //
	...
	WORKER extends TransferEngineWorker<LISTENER, RESULT, EXCEPTION, SESSION, VALUE, CONTEXT, CONTEXT_FACTORY, DELEGATE_FACTORY> //
> {
	...
	private static synchronized <FACTORY extends TransferEngineWorkerFactory<?, ?, ?, ?, ?, ?, ?, ?, ?>> void registerSubclass(
		Class<FACTORY> subclass) {

		final String key = subclass.getCanonicalName();
		Map<String, Object> m = TransferEngineWorkerFactory.REGISTRY.get(key);
		if (m != null) { return; }
		...
		TransferEngineWorkerFactory.REGISTRY.put(key, m);
		TransferEngineWorkerFactory.LOCATORS.put(key, locator);
	}

	protected static synchronized <FACTORY extends TransferEngineWorkerFactory<?, ?, ?, ?, ?, ?, ?, ?, ?>> FACTORY getTransferEngineWorkerFactory(
		...
		TransferEngineWorkerFactory.registerSubclass(subclass);
		Map<String, Object> m = TransferEngineWorkerFactory.REGISTRY.get(subclass.getCanonicalName());
	...
	public abstract WORKER newInstance(final Logger output, final WarningTracker warningTracker, final File baseData,
		...
		CmfCrypt crypto, String cfgNamePrefix, boolean supportsDuplicateNames) throws EXCEPTION;
