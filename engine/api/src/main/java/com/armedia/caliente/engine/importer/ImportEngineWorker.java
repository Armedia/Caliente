 ...
 *
 */
public abstract class ImportEngineWorker<//
	SESSION, //
...
> extends
	TransferEngineWorker<ImportEngineListener, ImportResult, ImportException, SESSION, VALUE, CONTEXT, CONTEXT_FACTORY, DELEGATE_FACTORY> {
	...
	protected ImportEngineWorker(Logger output, WarningTracker warningTracker, File baseData,
	...
	private final void renameObjectsWithMap(final Logger output, Properties p, CmfObjectStore<?, ?> objectStore,
		...
		for (String key : p.stringPropertyNames()) {
			...
			Matcher matcher = ImportEngineWorker.MAP_KEY_PARSER.matcher(key);
			if (!matcher.matches()) {
				continue;
			}
