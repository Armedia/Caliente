...
TransferEngine<S, T, V, C, ImportEngineListener> {
	...
	public final StoredObjectCounter<ImportResult> runImport(final Logger output, final ObjectStore<?, ?> objectStore,
		...
		try {
			...
			throw new ImportException("Failed to configure this import engine instance", e);
		}
			...
			try {
						...
						try {
							while (!Thread.interrupted()) {
								...
								try {
									...
									if (this.log.isDebugEnabled()) {
										this.log.debug(String.format("Polled a batch with %d items",
											batch.contents.size()));
									}
									try {
									...
									for (StoredObject<?> next : batch.contents) {
										if (failBatch) {
											...
											this.log.error(String.format(
												"Batch has been failed - will not process [%s](%s) (%s)",
												next.getLabel(), next.getId(), result.name()));
											...
											try {
												...
												if (this.log.isDebugEnabled()) {
													...
													switch (outcome.getResult()) {
														...
														default:
															msg = String.format("Persisted (%s) %s",
																outcome.getResult(), next);
															break;
													}
													this.log.debug(msg);
												}
				...
				try {
					...
					for (final StoredObjectType type : StoredObjectType.values()) {
						if (!isSupported(type)) {
							this.log.debug(String.format("Ignoring objects of type %s due to engine configuration",
								type));
							continue;
						}
						...
						if (strategy.isIgnored()) {
							if (this.log.isDebugEnabled()) {
								this.log
								.debug(String.format(
									"Skipping type %s because it marked as ignored by the given strategy",
									type.name()));
							}
							continue;
						}
						...
						this.log.info(String.format("%d %s objects available, starting deserialization", total,
							type.name()));
						try {
							objectStore.loadObjects(translator, type, handler);
						} catch (Exception e) {
							throw new ImportException(String.format(
								"Exception raised while loading objects of type [%s]", type), e);
						}
						...
						if (abortImport(type, errorCount)) {
							this.log.info(String.format(
								"Import aborted due to %d errors detected while importing objects of type %s",
