...
public class AlfFile {

	private static final String ENCODING = "UTF-8";

	private static String makeSafe(String s) throws IOException {
		return URLEncoder.encode(s, AlfFile.ENCODING);
	}

	private static String makeUnsafe(String s) throws IOException {
		return URLDecoder.decode(s, AlfFile.ENCODING);
	}

	public static String decodeSafePath(String safePath) throws IOException {
		List<String> r = new ArrayList<>();
		for (String s : FileNameTools.tokenize(safePath, '/')) {
			r.add(AlfFile.makeUnsafe(s));
		}
		return AlfRoot.normalize(FileNameTools.reconstitute(r, false, false));
	}

	public static AlfFile newFromSafePath(AlfRoot root, String safePath) throws IOException {
		return new AlfFile(root, AlfFile.decodeSafePath(safePath));
	}
	...
	public AlfFile(AlfRoot root, String path) throws IOException {
		this.root = root;
	...
	public boolean equals(Object obj) {
		...
		AlfFile other = AlfFile.class.cast(obj);
	...
	public String toString() {
		return String.format(
			"AlfFile [root=%s, absoluteFile=%s, relativeFile=%s, safePath=%s, path=%s, name=%s, pathCount=%s]",
