...
public class MimeTools {
	...
	private static final Map<String, Boolean> MIME_VALID = new ConcurrentHashMap<String, Boolean>();
	private static final Map<String, Reference<MimeType>> MIME_CACHE = new ConcurrentHashMap<String, Reference<MimeType>>();
	...
	static {
		...
		if (MimeTools.DEFAULT_MIME_TYPE == null) { throw new RuntimeException(String.format(
			"Failed to parse the default mime string [%s]", MimeTools.DEFAULT_MIME_STRING)); }
	...
	public static MimeType resolveMimeType(String type) {
		...
		if ((valid == null) || (ret == null) || (ret.get() == null)) {
			synchronized (MimeTools.MIME_LOCK) {
				...
				if ((valid == null) || (ret == null) || (ret.get() == null)) {
					try {
						ret = new WeakReference<MimeType>(new MimeType(type));
	 ...
	 * @return the correct {@link MimeType} instance detected from the given data
	 */
	public static MimeType determineMimeType(InputStream data) {
		return MimeTools.convert(MimeTools.cast(MimeUtil.getMimeTypes(data).iterator().next()));
	}
