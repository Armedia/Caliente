...
public class DctmACL {
	...
	public static final String PERMIT_VALUE = "permitValue";

	private static final Logger LOG = LoggerFactory.getLogger(DctmACL.class);
	...
	private static Set<String> calculateActionsForPermissions(int permit, String extended) {
		...
		for (int i = permit; i >= IDfACL.DF_PERMIT_NONE; i--) {
			actions = DctmACL.PERMIT_TO_ACTION.get(i);
			if (actions != null) {
				ret.addAll(actions);
			}
		}
		...
		if (extended != null) {
			for (String xp : FileNameTools.tokenize(extended, ',')) {
				actions = DctmACL.XPERMIT_TO_ACTION.get(xp);
				if (actions != null) {
					ret.addAll(actions);
				}
			}
	...
	public static Collection<IDfPermit> calculatePermissionsForActor(CmfActor actor) {
		...
		if (actions != null) {
			...
			for (String a : actions) {
				Integer newPerm = DctmACL.ACTION_TO_PERMIT.get(a);
				if ((newPerm != null) && (newPerm.intValue() > perm)) {
					perm = newPerm.intValue();
				}
				String e = DctmACL.ACTION_TO_XPERMIT.get(a);
				if (e != null) {
					extended.add(e);
				}
			}
	...
	public static CmfACL<IDfValue> calculateACL(final IDfACL acl) throws DfException, ExportException {
		...
		for (int i = 0; i < permitCount; i++) {
			...
			if ((o == null) && !DctmMappingUtils.SPECIAL_NAMES.contains(accessor)) {
				...
				if (!missingAccessors.contains(accessor)) {
					DctmACL.LOG.warn(String.format(
		...
		for (int i = 0; i < accCount; i++) {
			...
			for (String action : DctmACL.calculateActionsForPermissions(accessorPermit, extendedPermits)) {
