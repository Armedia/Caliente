...
import com.armedia.cmf.engine.alfresco.bulk.importer.jaxb.AclPermitT;
import com.armedia.cmf.engine.alfresco.bulk.importer.jaxb.AclT;
import com.armedia.cmf.engine.alfresco.bulk.importer.jaxb.AclsT;
import com.armedia.cmf.engine.alfresco.bulk.importer.jaxb.PermitTypeT;
...
public class AlfAclImportDelegate extends AlfAggregatedImportDelegate<AclT, AclsT> {
	...
	protected AlfAclImportDelegate(AlfImportDelegateFactory factory, CmfObject<CmfValue> storedObject)
		throws Exception {
		super(factory, storedObject, AclsT.class);
	}

	@Override
	protected AclT createItem(CmfAttributeTranslator<CmfValue> translator, AlfImportContext ctx)
		...
		for (int i = 0; i < entries; i++) {
			...
			str.setLength(0);
			for (Integer b : AlfAclImportDelegate.XPERMITS.keySet()) {
				if (bits.get(b)) {
					if (str.length() > 0) {
						str.append(',');
					}
					str.append(AlfAclImportDelegate.XPERMITS.get(b));
				}
			}
