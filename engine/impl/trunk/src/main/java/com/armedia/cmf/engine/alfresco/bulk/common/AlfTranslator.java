...
public class AlfTranslator extends CmfAttributeTranslator<CmfValue> {
	...
	static {
		...
		// am.put(AlfAttributes.VERSION_ANTECEDENT_ID.name,
	...
	public static CmfDataType decodePropertyType(PropertyType t) {
		return AlfTranslator.DATA_TYPES.get(t);
	}

	public static PropertyType decodePropertyType(CmfDataType t) {
		return AlfTranslator.DATA_TYPES_REV.get(t);
	}
	...
	private BidiMap<String, IntermediateAttribute> getAttributeMappings(CmfType type) {
		return AlfTranslator.ATTRIBUTE_MAPPINGS.get(type);
	}
