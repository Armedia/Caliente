...
public class LocalTranslator extends ObjectStorageTranslator<StoredValue> {
	...
	static {
		...
		pm.put(LocalProperties.TARGET_PATHS.name, IntermediateProperty.PATH);
		pm.put(LocalProperties.TARGET_PARENTS.name, IntermediateProperty.PARENT_ID);
		pm.put(LocalProperties.CONTENTS.name, IntermediateProperty.CONTENT_STREAM_ID);
		pm.put(LocalProperties.CURRENT_VERSION.name, IntermediateProperty.CURRENT_VERSION);
	...
	private BidiMap<String, IntermediateProperty> getPropertyMappings(StoredObjectType type) {
		return LocalTranslator.PROPERTY_MAPPINGS.get(type);
	}
