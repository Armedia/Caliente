...
public class XmlRecursiveIterator implements Iterator<ExportTarget> {
	...
	public XmlRecursiveIterator(XmlRoot root, boolean excludeEmptyFolders) throws IOException {
		this.root = root;
		...
		recursion: while (!this.stateStack.isEmpty()) {
			...
			if (state.childIterator != null) {
				while (state.childIterator.hasNext()) {
					...
					try {
						state.next = new XmlFile(this.root, f.getPath());
					} catch (IOException e) {
