...
public class DctmExportEngineTest {
	...
	public void test() throws Exception {
		...
		settings.put(Setting.DQL.getLabel(), "select r_object_id, 'dm_type' as r_object_type from dm_type");
		engine.runExport(output, objectStore, streamStore, settings);

		settings.put(Setting.DQL.getLabel(), "select r_object_id, 'dm_format' as r_object_type from dm_format");
		engine.runExport(output, objectStore, streamStore, settings);
