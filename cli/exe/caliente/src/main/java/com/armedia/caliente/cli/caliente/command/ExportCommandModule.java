...
public class ExportCommandModule extends CommandModule<ExportEngineFactory<?, ?, ?, ?, ?, ?>> {
	...
	protected int execute(CalienteState state, OptionValues commandValues, Collection<String> positionals)
		...
		Set<CmfArchetype> types = commandValues.getEnums(CmfArchetype.class, CfgTools.ignoreFailures(), CLIParam.manifest_types);
		if (types == null) {
			types = EnumSet.allOf(CmfArchetype.class);
		}
		...
		final Date start;
		final Date end;
		Map<CmfArchetype, Long> summary = null;
		...
		if (summary != null) {
			...
			long total = 0;
			for (CmfArchetype t : summary.keySet()) {
