...
public enum CLIParam {
	...
	threads(1, "The number of threads to use (defaults to half the number of logical processors)"),
	lib(
		1,
	...
	docbase(1, true, "The repostory name to connect to within the target server (optional)"),
	user(1, true, "The username to connect with"),
	password(1, true, "The password to connect with"),
	target(1, true, "The root folder within which to create the data"),
	object_types(-1, "Names of the object types to generate samples for (must be subtypes of dm_sysobject)"),
	tree_depth(1, "The depth of the tree (defaults to 1)"),
	folder_count(1, "The number of folders inside each folder (defaults to one per folder type included)"),
	document_count(1, "The number of documents inside each folder (defaults to one per non-folder type included)"),
	document_min_size(
		1,
		"The minimum size for the (random) content stream for each document (min = 1 byte).  Suffixes such as KB and MB are supported - no suffix = bytes."),
	document_max_size(
		1,
		"The maximum size for the (random) content stream for each document (capped out at 16MB). Suffixes such as KB and MB are supported - no suffix = bytes."),
	name_format(
		1,
	...
	public List<Long> getAllLong() {
		List<String> l = getAllString();
		if ((l == null) || l.isEmpty()) { return Collections.emptyList(); }
		List<Long> r = new ArrayList<Long>(l.size());
		for (String s : l) {
