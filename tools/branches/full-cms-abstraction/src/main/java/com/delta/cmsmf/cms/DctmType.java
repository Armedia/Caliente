 ...
 *
 */
public class DctmType extends DctmPersistentObject<IDfType> {
	...
	private static synchronized void initSpecialTypes() {
		if (DctmType.SPECIAL_TYPES_READY) { return; }
		String specialTypes = Setting.SPECIAL_TYPES.getString();
		StrTokenizer strTokenizer = StrTokenizer.getCSVInstance(specialTypes);
		DctmType.SPECIAL_TYPES = Collections.unmodifiableSet(new HashSet<String>(strTokenizer.getTokenList()));
		DctmType.SPECIAL_TYPES_READY = true;
	}

	public static boolean isSpecialType(String type) {
		DctmType.initSpecialTypes();
		return DctmType.SPECIAL_TYPES.contains(type);
	}

	public DctmType() {
		super(IDfType.class);
		DctmType.initHandlers();
		DctmType.initSpecialTypes();
	}
	...
	@Override
	protected void doPersistRequirements(IDfType type, DctmTransferContext ctx, DctmDependencyManager manager)
		throws DfException, CMSMFException {
		IDfType superType = type.getSuperType();
		if (superType == null) { return; }
		if (DctmType.isSpecialType(superType.getName())) {
	...
	@Override
	protected IDfId persistChanges(IDfType object, DctmTransferContext context) throws DfException, CMSMFException {
	...
	@Override
	protected void prepareForConstruction(IDfType object, boolean newObject, DctmTransferContext context)
		throws DfException {
	}

	@Override
	protected boolean isValidForLoad(IDfType type) throws DfException {
		if (DctmType.isSpecialType(type.getName())) { return false; }
