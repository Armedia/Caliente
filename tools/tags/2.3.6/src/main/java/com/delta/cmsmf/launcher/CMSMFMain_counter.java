 ...
 */
public class CMSMFMain_counter extends AbstractCMSMFMain {
	...
	DfException {
		...
		while (result.next()) {
			children.add(result.getString(CmsAttributes.R_OBJECT_ID));
		}
		DfUtils.closeQuietly(result);
		...
		try {
			...
			try {
				...
				try {
					...
					this.log.fatal("Exception caught while aborting the transaction for object counts", e);
				}
