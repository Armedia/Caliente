...
public class CmsObjectStore {
	...
	public void deserializeObjects(Class<? extends CmsObject<?>> klass, Set<String> ids, ObjectHandler handler)
		...
		try {
			...
			try {
				...
				try {
					while (objRS.next()) {
						final long objNum = objRS.getLong("object_number");
						...
						// ordering factor. We use 0-padded hex to ensure proper ordering.
	...
	public boolean isSerialized(CmsObjectType type, String objectId) throws CMSMFException {
		final Connection c;
		try {
			c = this.dataSource.getConnection();
		} catch (SQLException e) {
	...
	private CmsObject<?> doPersistDfObject(IDfPersistentObject dfObject, final CmsExportContext ctx)
		...
		if (!obj.loadFromCMS(dfObject, ctx)) {
