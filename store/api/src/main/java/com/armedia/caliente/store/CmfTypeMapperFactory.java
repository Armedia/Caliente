...
public abstract class CmfTypeMapperFactory {
	...
	static {
		Map<String, CmfTypeMapperFactory> factories = new HashMap<>();
		PluggableServiceLocator<CmfTypeMapperFactory> locator = new PluggableServiceLocator<>(
		...
		locator.setErrorListener(new PluggableServiceLocator.ErrorListener() {
			...
			public void errorRaised(Class<?> serviceClass, Throwable t) {
				CmfTypeMapperFactory.LOG
					.warn(String.format("Error raised initializing CmfTypeMapperFactory subclass [%s]",
	...
	protected CmfTypeMapperFactory(String name) {
		...
		if (!CmfTypeMapperFactory.VALIDATOR.matcher(name)
